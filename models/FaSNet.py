...
class SingleRNN(nn.Module):
    ...
    args:
        rnn_type: string, select from 'RNN', 'LSTM' and 'GRU'. or Transformer
    ...
    def __init__(self, rnn_type, input_size, hidden_size, dropout=0, bidirectional=False, dim_feedforward=512, num_layers=4):
        ...
        if self.rnn_type != "transformer_encoder":
    ...
    def forward(self, input):
        ...
        if self.rnn_type != "transformer_encoder":
            ...
            # print("rnn", output.shape, rnn_output.shape) # rnn torch.Size([8, 94, 384]) torch.Size([8, 94, 256])
            ...
            output = input
            rnn_output = self.transformer(output)
            # print(f"rnn_output:{rnn_output.shape}")
            return rnn_output
